...
const PATTERNS = {
  ...
  insee: {
    regex:
      /\b[1-478]\s?\d{2}\s?(?:0[1-9]|1[0-2]|[2-9]\d)\s?(?:\d{2}|2[AB])\s?\d{3}\s?\d{3}(?:\s?\d{2})?\b/g,
    name: "INSEE",
  ...
  fr_streetAddress: {
    regex:
      /\b\d+\s*(?:bis|ter|quater)?\s+(?:rue|avenue|av.?|boulevard|bd|place|impasse|all[ée]e|chemin|quai|cours|passage|route|square|voie|esplanade)\s+[A-Za-zÀ-ÿ0-9'\-\s]+\b/gi,
    name: "fr_streetAddress",
  ...
  routingNumber: {
    regex: /\b(?:0\d|1[0-2]|2[1-9]|3[0-2]|6[1-9]|7[0-2]|80)\d{7}\b/g,
    name: "routingNumber",
